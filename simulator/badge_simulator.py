...
class _Gradient:
    """Linear / radial gradient brush, matching the badge's
    `brush.gradient(kind, x0, y0, x1, y1, stops, matrix)` API.

    The control points live in brush space; the optional brush matrix and the
    drawn shape's own transform are both folded in when the gradient is
    rasterised, exactly as the hardware brush does."""

    LINEAR = "linear"
    RADIAL = "radial"

    def __init__(self, kind, x0, y0, x1, y1, stops, matrix=None):
        self.kind = kind
        self.stops = self._normalise_stops(stops)
        self.geometry(x0, y0, x1, y1, matrix)

    @staticmethod
    def _normalise_stops(stops):
        normalised = []
        for position, value in stops:
            ...
            if len(value) == 3:
                value += (255,)
            normalised.append((float(position), value))
        normalised.sort(key=lambda stop: stop[0])
        return normalised or [(0.0, (255, 255, 255, 255))]

    def geometry(self, x0, y0, x1, y1, matrix=None):
    ...
    def color_at(self, t):
        ...
        for index in range(1, len(stops)):
            position, value = stops[index]
            if t <= position:
                prev_position, prev_value = stops[index - 1]
                span = position - prev_position
                ratio = 0.0 if span <= 0 else (t - prev_position) / span
                return tuple(
                    int(round(prev_value[i] + (value[i] - prev_value[i]) * ratio))
                    for i in range(4)
                )
        return stops[-1][1]

    def _endpoints(self, transform, offset):
        points = [(self.x0, self.y0), (self.x1, self.y1)]
        if isinstance(self.matrix, Matrix):
            points = [self.matrix.transformed_point(px, py) for px, py in points]
    ...
    def render(self, size, transform=None, offset=(0.0, 0.0)):
        """Rasterise the gradient across a surface of `size`."""
        layer = pygame.Surface(size, pygame.SRCALPHA)
        (px0, py0), (px1, py1) = self._endpoints(transform, offset)
        dx, dy = px1 - px0, py1 - py0
        length = math.hypot(dx, dy)
        if length < 0.001:
            # Degenerate axis: every point is at or beyond the end of the
            # ramp, so the final stop covers the whole area.
            layer.fill(self.color_at(1.0))
            return layer

        if self.kind == _Gradient.RADIAL:
            layer.fill(self.color_at(1.0))
            steps = max(8, min(96, int(length)))
            for step in range(steps, 0, -1):
                t = step / steps
                radius = int(round(length * t))
                if radius <= 0:
                    continue
                pygame.draw.circle(
                    layer,
        ...
        # Linear: build a one-pixel-tall colour ramp that covers the whole
        # surface, then rotate it onto the gradient axis.
        width, height = size
        span = int(math.hypot(width, height)) * 2 + 2
        strip = pygame.Surface((span, 1), pygame.SRCALPHA)
        half = span / 2
        for x in range(span):
            # distance along the axis, measured from the start point
            distance = x - half
            strip.set_at((x, 0), self.color_at(distance / length))
        strip = pygame.transform.scale(strip, (span, span))
        strip = pygame.transform.rotate(strip, -math.degrees(math.atan2(dy, dx)))
        rect = strip.get_rect(center=(int(round(px0)), int(round(py0))))
        layer.blit(strip, rect.topleft)
        return layer


def _render_gradient_shape(surface, gradient, shape, transform, offset):
    size = surface.get_size()
    mask = pygame.Surface(size, pygame.SRCALPHA)
    _render_shape(mask, (255, 255, 255, 255), shape, transform, offset)
    bounds = mask.get_bounding_rect()
    if bounds.width == 0 or bounds.height == 0:
        return
    if transform is None:
        base_shape = shape.shape if isinstance(shape, _StrokedShape) else shape
        transform = getattr(shape, "transform", None) or getattr(base_shape, "transform", None)
    layer = gradient.render(size, transform, offset)
    layer.blit(mask, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
    surface.blit(layer, bounds, bounds)


def _render_shape(surface, color, shape, transform=None, offset=(0.0, 0.0)):
    if isinstance(color, _Gradient):
        _render_gradient_shape(surface, color, shape, transform, offset)
        return
...
def _scaled_color(color, alpha):
    if alpha >= 255:
        return color
    return (color[0], color[1], color[2], int(color[3] * alpha / 255))


def _draw_brush_shape(surface, brush, shape, alpha=255, offset=(0.0, 0.0)):
    """Draw `shape` with a solid colour or gradient brush, blending when the
    result is translucent (pygame's shape drawing would otherwise overwrite the
    destination alpha instead of compositing)."""
    alpha = max(0, min(255, int(alpha)))
    if alpha == 0:
        return
    if isinstance(brush, _Gradient):
        if alpha >= 255:
            _render_shape(surface, brush, shape, offset=offset)
            return
        layer = pygame.Surface(surface.get_size(), pygame.SRCALPHA)
        _render_shape(layer, brush, shape, offset=offset)
        layer.fill((255, 255, 255, alpha), special_flags=pygame.BLEND_RGBA_MULT)
    else:
        color = _scaled_color(brush, alpha)
        if color[3] >= 255:
            _render_shape(surface, color, shape, offset=offset)
            return
        layer = pygame.Surface(surface.get_size(), pygame.SRCALPHA)
        _render_shape(layer, color, shape, offset=offset)
...
class _SurfaceTarget:
    __slots__ = ("_surface", "brush", "font", "antialias", "alpha")
    ...
    def __init__(self, surface: pygame.Surface):
        ...
        self.antialias = 0
        # Global opacity applied to everything drawn, like the badge's
        # `screen.alpha`.
        self.alpha = 255
    ...
    @property
    def clip(self):
        return (0, 0, self._surface.get_width(), self._surface.get_height())
    ...
    def _norm_color(self, c):
        ...
        if isinstance(c, int):
            return (c, c, c, 255)
        if isinstance(c, _Gradient):
            return c
        components = tuple(c)
        if len(components) == 4:
            return components
        if len(components) == 3:
            return components + (255,)
        raise ValueError(f"Expected an (r, g, b) or (r, g, b, a) colour, got {c!r}")

    def _solid_color(self, c):
        """Resolve a brush to a single colour. A gradient has no single colour,
        so its midpoint is sampled; unlike the hardware, the simulator draws
        flat text and clears rather than gradient-filled ones."""
        color = self._norm_color(c)
        if isinstance(color, _Gradient):
            color = color.color_at(0.5)
        return _scaled_color(color, self.alpha)

    def _unwrap(self, image):
        return image._surface if isinstance(image, Image) else image

    def clear(self, color=None) -> None:
        fill_color = self._solid_color(color if color is not None else self.brush)
        self._surface.fill(fill_color)

    def draw(self, shape: _Shape) -> None:
        _draw_brush_shape(
            self._surface, self._norm_color(self.brush), shape, self.alpha
        )

    def shape(self, value: _Shape) -> None:
        self.draw(value)

    def rectangle(self, *args) -> None:
        if len(args) == 4:
            self.draw(shapes.rectangle(*args))
        elif len(args) == 1 and isinstance(args[0], tuple):
            self.draw(shapes.rectangle(*args[0]))
        elif len(args) == 1:
            self.draw(args[0])
        else:
            raise TypeError("rectangle() expects x, y, w, h or a shape")

    def blit(self, image, x: float, y=None, transform: "Matrix" = None) -> None:
        if isinstance(x, Rect):
            src = self._unwrap(image)
            dest = y if isinstance(y, Rect) else x
            if isinstance(y, Rect):
                source = pygame.Rect(
                    int(x.x), int(x.y), max(1, abs(int(x.w))), max(1, abs(int(x.h)))
                )
                src = src.subsurface(source)
            scaled = pygame.transform.scale(
                src, (max(1, abs(int(dest.w))), max(1, abs(int(dest.h))))
            )
            if dest.w < 0:
                scaled = pygame.transform.flip(scaled, True, False)
            if dest.h < 0:
                scaled = pygame.transform.flip(scaled, False, True)
            self._surface.blit(scaled, (int(round(dest.x)), int(round(dest.y))))
            return
    ...
    def text(self, text: str, x: float, y: float) -> None:
        font = self.font
        color = self._solid_color(self.brush)
...
class Image(_SurfaceTarget):
    ...
    def load(path: str):
        ...
        image = Image(_surface=source.copy())
        image._source_path = path
        return image

    def spritesheet(self, columns, rows):
        if self._source_path is None:
            raise ValueError("spritesheet() requires an image loaded from a path")
        return SpriteSheet(self._source_path, columns, rows)
...
class Screen(_SurfaceTarget):
    ...
    def set_mode(self, flags: int) -> None:
        width, height = (320, 240) if flags & HIRES else (160, 120)
        if self.width == width and self.height == height:
            return
        self.width = width
        self.height = height
        self._surface = pygame.Surface((width, height), pygame.SRCALPHA)
        self._toolbar_cache = (None, None, None)
        self._apply_window_mode()
...
class _Window:
    ...
    def clear(self, color=None):
        clip = self._set_clip()
        try:
            fill_color = self._parent._solid_color(color if color is not None else self.brush)
            rect = pygame.Rect(self.x, self.y, self.width, self.height)
    ...
    def draw(self, shape: _Shape) -> None:
        ...
        try:
            _draw_brush_shape(
                self._parent._surface,
                self._parent._norm_color(self.brush),
                shape,
                self.alpha,
    ...
    def text(self, text: str, x: float, y: float) -> None:
        ...
        try:
            ...
            surf = font.render(str(text), True, self._parent._solid_color(self.brush))
...
class IO:
    ...
    def __init__(self) -> None:
        ...
        self._key_map = {
            ...
            pygame.K_BACKSPACE: ("BUTTON_BACK",),
            pygame.K_TAB: ("BUTTON_MENU",),
...
class BadgeAPI:
    ...
    BUTTON_LEFT = IO.BUTTON_LEFT
    BUTTON_RIGHT = IO.BUTTON_RIGHT
    BUTTON_SELECT = IO.BUTTON_B
    BUTTON_BACK = "BUTTON_BACK"
    BUTTON_MENU = "BUTTON_MENU"
    BUTTON_HOME = IO.BUTTON_HOME

    def __init__(self, io_ref):
        self._io = io_ref

    @property
    def ticks(self):
        return self._io.ticks

    @property
    def ticks_delta(self):
        return self._io.ticks_delta

    def pressed(self, action=None):
        return set(self._io.pressed) if action is None else action in self._io.pressed

    def held(self, action=None):
        return set(self._io.held) if action is None else action in self._io.held

    def released(self, action=None):
        return set(self._io.released) if action is None else action in self._io.released

    def touched(self, action):
        return self.held(action)

    def direction(self):
        x = int(self.held(self.BUTTON_RIGHT)) - int(self.held(self.BUTTON_LEFT))
        y = int(self.held(self.BUTTON_DOWN)) - int(self.held(self.BUTTON_UP))
        length = math.hypot(x, y)
        return Vec2(x / length, y / length) if length else Vec2()
...
class _FontAPI:
    def __getattr__(self, name):
        return PixelFont.load(f"/system/assets/fonts/{name}.ppf")


class _BrushAPI:
    LINEAR = _Gradient.LINEAR
    RADIAL = _Gradient.RADIAL

    @staticmethod
    def gradient(kind, x0, y0, x1, y1, stops, matrix=None):
        return _Gradient(kind, x0, y0, x1, y1, stops, matrix)
...
def fatal_error(title: str, message: str = "") -> None:
    raise RuntimeError(f"{title}: {message}" if message else str(title))
...
class _MockWLAN:
    ...
    def __init__(self, interface_id):
        ...
        try:
            ...
            if sim_root:
                secrets_file = os.path.join(sim_root, "secrets.py")
...
def run(update_func, fps: int = 60, init=None, on_exit=None):
    if globals().get("_loading_module", False):
...
def load_game_module(module_path: str) -> ModuleType:
    ...
    badgeware.fatal_error = fatal_error
    ...
    # Provide urllib.urequest without replacing Python's standard urllib package.
    urequest_module = ModuleType("urllib.urequest")
    urequest_module.urlopen = _MockUrequest.urlopen
    
    import urllib
    urllib.urequest = urequest_module
    
    # Register the MicroPython-compatible submodule.
    ...
    wifi_module = ModuleType("wifi")
    wifi_module.connect = lambda: False
    ...
    mod.__dict__.update(
        ...
        mat3=Matrix,
        run=run,
        BUTTON_UP=badge_api.BUTTON_UP,
        BUTTON_DOWN=badge_api.BUTTON_DOWN,
        BUTTON_LEFT=badge_api.BUTTON_LEFT,
        BUTTON_RIGHT=badge_api.BUTTON_RIGHT,
        BUTTON_SELECT=badge_api.BUTTON_SELECT,
        BUTTON_BACK=badge_api.BUTTON_BACK,
        BUTTON_MENU=badge_api.BUTTON_MENU,
        BUTTON_HOME=badge_api.BUTTON_HOME,
        HIRES=HIRES,
        VSYNC=VSYNC,
        file_exists=file_exists,
        is_dir=is_dir,
        fatal_error=fatal_error,
        launch=lambda path: path,
        reset=lambda: None,
    )
    runtime_globals = {
        "badge": badge_api,
...
def _unload_app_modules(game_dir: str) -> None:
    ...
    game_dir_abs = os.path.abspath(game_dir) if game_dir else None
    if game_dir:
    ...
    for mod_name, mod in sys.modules.items():
        if mod and hasattr(mod, "__file__") and mod.__file__:
            ...
            if game_dir_abs and mod_file.startswith(game_dir_abs):
...
def main() -> None:
    global _perf_monitor, screen, io, SIM_ROOT
    ...
    # Resolve the virtual /system root before initializing pygame so --clean
    # can remove persisted state immediately.
    if args.system_root:
    ...
    if args.clean:
        ...
        state_dir = os.path.join(SIM_ROOT, ".badge_state")
        if os.path.exists(state_dir):
            try:
                shutil.rmtree(state_dir)
                print(f"Cleaned persisted simulator state: {state_dir}")
            except Exception as e:
                print(f"Warning: Could not clean persisted simulator state: {e}")
    ...
    while True:
        ...
        # Each app starts in the default 160x120 mode; an app that switched to
        # HIRES must not leave the next app (or the menu) on a 320x240 surface.
        screen.set_mode(0)
