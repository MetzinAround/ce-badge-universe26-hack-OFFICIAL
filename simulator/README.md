...
`badge_simulator.py` lets you run badge apps locally by reproducing the Universe
2026 `badge` API on top of Pygame. Use it to iterate quickly or to smoke-test
changes before shipping them to the hardware badge.

## Browser simulator

For a no-install preview, use the
[Pimoroni Badgeware Web Simulator](https://pimoroni.github.io/badgeware-web-simulator/).
It is the preferred first look for the upstream Badgeware experience. The
local simulator remains useful for testing files in this repository and for
running repeatable checks from the command line.

The browser simulator and the local simulator are development aids. Neither
one proves BLE, GPIO, IR electrical behavior, battery behavior, radio
conditions, exact memory use, or physical input behavior on the Universe 2026
badge.

## Prerequisites
- Python 3.10 or newer.
- Pygame and psutil from the root `requirements.txt`.
...
- Provides logical `BUTTON_*` actions, `badge.ticks_delta`, display-mode
  constants, orientation-safe direction input, and a neutral IMU model
...
3. The app follows the same UI flow, but real WiFi association, TLS memory use, timing, and failure handling still require testing on hardware.
...
## Controls
- Arrow keys → directional logical actions
- `Enter` → SELECT
- `Backspace` → BACK
- `Tab` → MENU
...
- ✅ Logical button mappings and input handling
- ✅ Frame rate (60 FPS)
...
**What's Different:**
- ❌ **Memory**: Desktop Python uses much more memory than MicroPython
